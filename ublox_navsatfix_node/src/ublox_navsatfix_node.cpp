#include "ublox_navsatfix_node.hpp"

#include <rcutils/logging_macros.h>


UbloxNavSatFix::UbloxNavSatFix() : Node("ublox_navsatfix_node")
{
    RCLCPP_INFO(get_logger(), "Ublox NavSatFix node starting");
    ...
    // Create subscribers
    ubx_nav_hp_pos_llh_sub_ = this->create_subscription<ublox_ubx_msgs::msg::UBXNavHPPosLLH>("ubx_nav_hp_pos_llh", qos,
        std::bind(&UbloxNavSatFix::nav_hp_pos_llh_callback, this, std::placeholders::_1));
    ubx_nav_cov_sub_ = this->create_subscription<ublox_ubx_msgs::msg::UBXNavCov>("ubx_nav_cov", qos,
        std::bind(&UbloxNavSatFix::nav_cov_callback, this, std::placeholders::_1));
    ubx_nav_status_sub_ = this->create_subscription<ublox_ubx_msgs::msg::UBXNavStatus>("ubx_nav_status", qos,
        std::bind(&UbloxNavSatFix::nav_sta_callback, this, std::placeholders::_1));
}

void UbloxNavSatFix::nav_hp_pos_llh_callback(const ublox_ubx_msgs::msg::UBXNavHPPosLLH::SharedPtr ubx_hppos_llh_msg)
{
    ...
    RCLCPP_DEBUG(this->get_logger(), "Published NavSatFix with lat %4f lon %4f alt %4f", lat, lon, alt); 

}

void UbloxNavSatFix::nav_cov_callback(const ublox_ubx_msgs::msg::UBXNavCov::SharedPtr ubx_cov_msg) 
{
    // 6 position covariance values available in UBX-NAV-COV matrix (symmetrix, so only upper triangular values shown)
    // pos_cov_nn
    // pos_cov_ne
    // pos_cov_nd
    // pos_cov_ee
...
void UbloxNavSatFix::nav_sta_callback(const ublox_ubx_msgs::msg::UBXNavStatus::SharedPtr ubx_sta_msg) 
{
...
int main(int argc, char * argv[])
{
  ...
  auto ublox_navsatfix_node = std::make_shared<UbloxNavSatFix>();
  rclcpp::spin(ublox_navsatfix_node);
  rclcpp::shutdown();
  return 0;
}
