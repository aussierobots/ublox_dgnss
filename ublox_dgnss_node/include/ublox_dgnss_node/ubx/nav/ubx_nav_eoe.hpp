...
#ifndef UBLOX_DGNSS_NODE__UBX_NAV_EOE_HPP
#define UBLOX_DGNSS_NODE__UBX_NAV_EOE_HPP
...
namespace ubx {
  namespace nav::eoe {
    ...
    class NavEOEPayload : UBXPayload {
      ...
      const static msg_id_t MSG_ID = UBX_NAV_EOE;

      u4_t iTOW;  // ms - GPS Time of week of the navigation epoch.

      public:
      NavEOEPayload() : UBXPayload(MSG_CLASS,MSG_ID) {}
      NavEOEPayload(ch_t* payload_polled, u2_t size) : UBXPayload(MSG_CLASS,MSG_ID) {
        payload_.clear();
...
#endif // UBLOX_DGNSS_NODE__UBX_NAV_EOE_HPP