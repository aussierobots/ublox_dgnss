...
#ifndef UBLOX_DGNSS_NODE__UBX_NAV_PVT_HPP
#define UBLOX_DGNSS_NODE__UBX_NAV_PVT_HPP
...
namespace ubx {
  namespace nav::pvt {
    ...
    struct validity_flags_t {
      union {
        ...
        struct
        {
          ...
          l_t fullyResolved:1;    // 1 = UTC Time of day has been fully resolved (no seconds uncertainty)
          l_t validMag:1;         // 1 = valid magnetic declination
        } bits;
      };
    };

    enum gnss_fix_t: u1_t {gnss_no_fix = 0, gnss_dead_reackoning_only = 1, gnss_fix_2d = 2, gnss_fix_3d = 3,
    ...
    enum pvt_carrier_solution_status_t: u1_t {no_carrier_range_solution = 0, carrier_phase_solution_with_floating_ambiguities = 1,
    ...
    struct gnss_fix_status_flags_t {
      union {
        ...
        struct 
        {
          ...
          l_t headVehValid:1;    // 1 = heading of vehicle is valid, only set if the receiver is in sensor fusion mode
    ...
    struct additional_flags_1_t {
      union {
        ...
        struct 
        {
          l_t confirmedAvailable:1; // 1 = information about UTC date and time of day validity confirmation is available 
    ...
    class NavPvtPayload : UBXPayload {
      ...
      const static msg_id_t MSG_ID = UBX_NAV_PVT;
      ...
      i4_t headVeh; // deg scale 1e-5 - Heading of vehicle (2-D), this is only valid when headVehValid is set, 
      ...
      u2_t magAcc; // deg scale ie-2 - Mangetic declination accuracy. Only supported in ADR 4.10 and later

      public:
      NavPvtPayload() : UBXPayload(MSG_CLASS,MSG_ID) {}
      NavPvtPayload(ch_t* payload_polled, u2_t size) : UBXPayload(MSG_CLASS,MSG_ID) {
        payload_.clear();
      ...
      std::string to_string() {
        ...
        oss << "iTOW: " << iTOW;
        oss << std::setfill('0');
        oss << " date time: " << std::setw(2) << +year << "/" << std::setw(2) << +month << "/" <<  std::setw(2) << +day;
        oss << " " << std::setw(2) << +hour << ":" << std::setw(2) << +min << ":" << std::setw(2) << +sec;
...
#endif // UBLOX_DGNSS_NODE__UBX_NAV_PVT_HPP