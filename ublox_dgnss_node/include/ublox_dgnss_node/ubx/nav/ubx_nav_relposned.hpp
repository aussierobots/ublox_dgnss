...
#ifndef UBLOX_DGNSS_NODE__UBX_NAV_RELPOSNED_HPP
#define UBLOX_DGNSS_NODE__UBX_NAV_RELPOSNED_HPP
...
namespace ubx {
  namespace nav::relposned {


    enum carrier_solution_status_t: u1_t {no_carrier_range_solution = 0, carrier_phase_solution_with_floating_ambiguities = 1,
    ...
    struct status_flags_t {
      union {
        ...
        struct 
        {
          ...
          l_t relPosValid:1; // 1 = relative position components and accuracies are valid and, in moving base mode only, if baseline is valid
          ...
          l_t refPosMiss:1; // 1 if extrapolated reference position was used to compute moving base solution this epoch. 
          l_t refObsMiss:1; // 1 if extrapolated reference observations were used to compute moving base solution this epoch. 
          l_t relPosHeadingValid:1; // 1 = relPosHeading is valid
          l_t relPosNormalized:1; // 1 = components of the relative position vector (including the high-precision parts) are normalized
        } bits;
      }; 
    };
    ...
    class NavRelPosNedPayload : UBXPayload {
      ...
      const static msg_id_t MSG_ID = UBX_NAV_RELPOSNED;
      ...
      i1_t relPosHPLength;  // mm scale 0.1 - full HP length is given by relPosLength + (relPosHPLength * 1e-2) 
      ...
      public:
      NavRelPosNedPayload() : UBXPayload(MSG_CLASS,MSG_ID) {}
      NavRelPosNedPayload(ch_t* payload_polled, u2_t size) : UBXPayload(MSG_CLASS,MSG_ID) {
        payload_.clear();
...
#endif // UBLOX_DGNSS_NODE__UBX_NAV_RELPOSNED_HPP