...
namespace ubx {
  ...
  struct Frame {
    ...
    std::string to_hex() {
      ...
      for (const auto &u1: buf) {
        // os << std::setfill('0') << std::setw(2) <<std::right << std::hex << +u1;
        os << ubx::to_hex(u1);
      }
      os << "'";
      return os.str();
    ...
    std::string payload_to_hex() {
      ...
      for (size_t i = 0; i < length; i++) {
        // os << std::setfill('0') << std::setw(2) <<std::right << std::hex << +(u1_t)payload[i];
  ...
  std::shared_ptr<FramePolled> get_polled_frame(std::shared_ptr<usb::Connection> usbc, std::shared_ptr<FramePoll> poll_frame) {
      ...
      if (len>0) {
        if (buf[0]==UBX_SYNC_CHAR_1 && buf[1]==UBX_SYNC_CHAR_2) {
          ...
          if (polled_frame->msg_class == UBX_ACK && polled_frame->msg_id == UBX_ACK_NAK){
            ...
            msg_oss << " sent poll_frame.msg_class: " << "0x" << std::setfill('0') << std::setw(2) << std::right << std::hex << +poll_frame->msg_class;
            msg_oss << " poll_frame.msg_id: " << "0x" << std::setfill('0') << std::setw(2) << std::right << std::hex << +poll_frame->msg_id;
            msg_oss << " repsonse polled_frame.msg_class: " << "0x" << std::setfill('0') << std::setw(2) << std::right << std::hex << +polled_frame->msg_class;
            msg_oss << " polled_frame.msg_id: " << "0x" << std::setfill('0') << std::setw(2) << std::right << std::hex << +polled_frame->msg_id;
  ...
  class UBXPayloadBase {
    ...
    UBXPayloadBase(msg_class_t msg_class, msg_id_t msg_id) : msg_class_(msg_class), msg_id_(msg_id){
      payload_.clear();
    }
    UBXPayloadBase() : msg_class_(-1), msg_id_(-1){}
  };  
  ...
  class UBXPayload : UBXPayloadBase {
    ...
    UBXPayload(msg_class_t msg_class, msg_id_t msg_id) : UBXPayloadBase(msg_class,msg_id), msg_class_(msg_class), msg_id_(msg_id) {
      payload_.clear();
    }
    UBXPayload() : msg_class_(-1), msg_id_(-1){}
    public:
    virtual std::tuple<const u1_t*, size_t> make_poll_payload() {
      payload_.clear();
  ...
  class UBXPayloadPoll : UBXPayloadBase {
    public:
    UBXPayloadPoll() : UBXPayloadBase(-1, -1){}
    UBXPayloadPoll(msg_class_t msg_class, msg_id_t msg_id) : UBXPayloadBase(msg_class, msg_id){}
    // zero length payload 
    msg_class_t msg_class() {return msg_class_;};
    msg_id_t msg_id() {return msg_id_;};
  };

      
  class UBXPayloadOutputPrint : UBXPayloadBase{
    public:
    UBXPayloadOutputPrint() : UBXPayloadBase(-1, -1){}
    UBXPayloadOutputPrint(msg_class_t msg_class, msg_id_t msg_id) : UBXPayloadBase(msg_class, msg_id){}
    // return a hex string
    std::string to_hex() {
      const u1_t* buf= payload_.data();
  ...
  class Payload : public T, public UBXPayloadOutputPrint {
    public:
    Payload(ch_t *payload_polled, u2_t size): T(payload_polled, size), UBXPayloadOutputPrint(T::MSG_CLASS,T::MSG_ID){}
    Payload() : T(), UBXPayloadOutputPrint(T::MSG_CLASS,T::MSG_ID) {}
  };

  template <typename T>
  class PayloadPoll: public T, public UBXPayloadPoll {
    public:
    PayloadPoll() : T(), UBXPayloadPoll(T::MSG_CLASS,T::MSG_ID) {}
  };
  ...
  class FrameContainer {
    ...
    msg_class_t msg_class() {return msg_class_;};
    msg_id_t msg_id() {return msg_id_;};
    std::shared_ptr<Frame> frame() {return frame_;};
    ...
    std::shared_ptr<Payload<T>> payload() {return payload_;};
    std::shared_ptr<PayloadPoll<T>> payload_poll() {return payload_poll_;};
    ...
    std::shared_ptr<ubx::FramePoll> make_frame_poll() {
      ...
      frame_poll_->payload = (ch_t *)payload;
      frame_poll_->length = payload_size;
      std::tie(frame_poll_->ck_a, frame_poll_->ck_b) = frame_poll_->ubx_check_sum(); // calculate the checksum
  ...
  class UBXFrameComms {
    ...
    public:
    UBXFrameComms (std::shared_ptr<U> comms) {
      comms_=comms;
