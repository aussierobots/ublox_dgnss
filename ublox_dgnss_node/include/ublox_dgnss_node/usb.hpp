...
namespace usb 
{
  ...
  class UsbException : public std::runtime_error { 
    public: 
      UsbException(std::string msg):std::runtime_error(msg) {}
  };
  ...
  private:
    ...
    int hotplug_attach_callback(libusb_context *ctx, libusb_device *dev, libusb_hotplug_event event, void *user_data);
    int hotplug_detach_callback(libusb_context *ctx, libusb_device *dev, libusb_hotplug_event event, void *user_data);


    std::shared_ptr<transfer_t> make_transfer_in();
    std::shared_ptr<transfer_t> make_transer_out(u_char* buf, size_t size);
    void submit_transfer(std::shared_ptr<transfer_t> transfer, const std::string msg = "Error submit transfer: ", bool wait_for_completed=true);
  ...
  public:
    ...
    void set_exception_callback(connection_exception_cb_fn exception_fb_fn) {exception_cb_fn_=exception_fb_fn;};
    void set_hotplug_attach_callback(hotplug_attach_cb_fn hp_attach_cb_fn) {hp_attach_cb_fn_=hp_attach_cb_fn;};
    void set_hotplug_detach_callback(hotplug_detach_cb_fn hp_detach_cb_fn) {hp_detach_cb_fn_=hp_detach_cb_fn;};
