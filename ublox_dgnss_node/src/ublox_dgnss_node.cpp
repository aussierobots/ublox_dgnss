...
enum ParamStatus {
  PARAM_INITIAL,    // default value parameter used ie not set by user (and sent to GPS) or retrieved from gps
  PARAM_USER,       // value set by user either overidden at startup or param set
  PARAM_LOADED,     // loaded from gps device - not all items have a value set or default value on the gps
...
public:
  ...
  // Node ("ublox_dgnss", rclcpp::NodeOptions(options).use_intra_process_comms(true))
  Node ("ublox_dgnss", rclcpp::NodeOptions(options).automatically_declare_parameters_from_overrides(true))
  // Node ("ublox_dgnss", options)
  {
    ...
    while (!parameters_client->wait_for_service(1s)) {
      if (!rclcpp::ok()) {
        RCLCPP_ERROR(get_logger(), "Interrupted while waiting for parameter client service. Exiting.");
        rclcpp::shutdown();
      }
    ...
    for (auto name: list_param_result.names) {
      ...
      if (!valid) {
        RCLCPP_WARN(get_logger(), "parameter supplied: %s is not recognised. Ignoring!", name.c_str());
      }
    } 
    ...
    ubx_nav_hp_pos_llh_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavHPPosLLH>("ubx_nav_hp_pos_llh", qos);
    ubx_nav_odo_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavOdo>("ubx_nav_odo", qos);
    ubx_nav_pos_ecef_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavPosECEF>("ubx_nav_pos_ecef", qos);
    ubx_nav_pos_llh_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavPosLLH>("ubx_nav_pos_llh", qos);
    ubx_nav_pvt_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavPVT>("ubx_nav_pvt", qos);
    ubx_nav_rel_pos_ned_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavRelPosNED>("ubx_nav_rel_pos_ned", qos);
    ubx_nav_status_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavStatus>("ubx_nav_status", qos);
    ubx_nav_time_utc_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavTimeUTC>("ubx_nav_time_utc", qos);
    ubx_nav_vel_ecef_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavVelECEF>("ubx_nav_vel_ecef", qos);
    ubx_nav_vel_ned_pub_ = this->create_publisher<ublox_ubx_msgs::msg::UBXNavVelNED>("ubx_nav_vel_ned", qos);
    
    // ros2 parameter call backs
    parameters_callback_handle_ = this->add_on_set_parameters_callback(std::bind(&UbloxDGNSSNode::on_set_parameters_callback, this, _1));

    usb::connection_in_cb_fn connection_in_callback = std::bind(&UbloxDGNSSNode::ublox_in_callback, this, _1); 
    usb::connection_out_cb_fn connection_out_callback = std::bind(&UbloxDGNSSNode::ublox_out_callback, this, _1); 
    usb::connection_exception_cb_fn connection_exception_callback = std::bind(&UbloxDGNSSNode::ublox_exception_callback, this, _1, _2); 
    usb::hotplug_attach_cb_fn usb_hotplug_attach_callback = std::bind(&UbloxDGNSSNode::hotplug_attach_callback, this);
    usb::hotplug_detach_cb_fn usb_hotplug_detach_callback = std::bind(&UbloxDGNSSNode::hotplug_detach_callback, this);

    std::string node_name (this->get_name());
    hot_start_service_ = this->create_service<ublox_ubx_interfaces::srv::HotStart>(node_name+"/hot_start", std::bind(&UbloxDGNSSNode::hot_start_callback, this, _1, _2));
    cold_start_service_ = this->create_service<ublox_ubx_interfaces::srv::ColdStart>(node_name+"/cold_start", std::bind(&UbloxDGNSSNode::cold_start_callback, this, _1, _2));
    warm_start_service_ = this->create_service<ublox_ubx_interfaces::srv::WarmStart>(node_name+"/warm_start", std::bind(&UbloxDGNSSNode::warm_start_callback, this, _1, _2));
    reset_odo_service_ = this->create_service<ublox_ubx_interfaces::srv::ResetODO>(node_name+"/reset_odo", std::bind(&UbloxDGNSSNode::reset_odo_callback, this, _1, _2));
...
private:
  ...
  rclcpp::TimerBase::SharedPtr handle_usb_events_timer_; // specific to libusb to to process events asynchronously
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void log_usbc() {
    RCLCPP_INFO(this->get_logger(),"usb vendor_id: 0x%04x product_id: 0x%04x bus: %03d address: %03d port_number: %d speed: %s num_interfaces: %u ep_data out: 0x%02x in: 0x%02x ep_comms in: 0x%02x", 
      usbc_->vendor_id(), 
      usbc_->product_id(), 
      usbc_->bus_number(), 
...
public:
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void set_or_declare_ubx_cfg_param(const ubx::cfg::ubx_cfg_item_t &ubx_ci, const ubx::value_t &ubx_value, bool is_initial = false){
    rclcpp::ParameterValue p_value;
    
    bool p_set = has_parameter(ubx_ci.ubx_config_item); // auto overrides is on, so if supplied as args/yaml already set

    switch (ubx_ci.ubx_type) {
      case ubx::L:
        p_value = rclcpp::ParameterValue((bool)ubx_value.l);
        break;
      case ubx::E1:
      case ubx::U1:
    ...
    // this following cache map contains values retrieved from the GPS device and needs to be set before
    // either declaring or setting the ROS2 parameter. The on set parameter callback functions use this 
    // cache map to determine if the values have changed. If they have then it sets the value on 
    // the gps device
    ...
    if (p_set) {
      ...
      if (d_value != p_value){
        ...
        RCLCPP_INFO(get_logger(),"cfg declare_parameter name: %s with arguments supplied value ", p.get_name().c_str());
      } else {
  ...
  rcl_interfaces::msg::SetParametersResult cfg_val_set_from_ubx_ci_p_state(ubx::cfg::ubx_cfg_item_t ubx_ci, param_state_t param_state) {
  ...
  rcl_interfaces::msg::SetParametersResult cfg_val_set_from_parameter(rclcpp::Parameter parameter) {
    ...
    for (const auto &kv : ubx::cfg::ubxKeyCfgItemMap){
      auto ubx_key_id  = kv.first;
      auto ubx_cfg_item = kv.second;
      // RCLCPP_INFO(get_logger(),"parameter name: '%s' being compared to '%s'", parameter.get_name().c_str(), ubx_cfg_item.ubx_config_item);
        ...
        try {
          switch(ubx_cfg_item.ubx_type) {
            ...
            case ubx::R8:
              ...
              break;
            default:
              RCLCPP_WARN(get_logger(), "on_set_parameters_callback cfg_item: %s type not defined", ubx_cfg_item.ubx_config_item);
          }
          ...
          for (size_t i = 0; i < ubx_key_id.storage_size(); i++)
            voss << +value.bytes[i];
          RCLCPP_DEBUG(get_logger(), "cfg_item: %s appending to queue cfg_val_set_key: %s value: %s storage_size: %lu", ubx_cfg_item.ubx_config_item, ubx_key_id.to_hex().c_str(), voss.str().c_str(), ubx_key_id.storage_size());

          ubx_cfg_->cfg_val_set_key_append(ubx_key_id, value);

          // RCLCPP_INFO(get_logger(), "cfg_val_set_cfgdata_size(): %d", ubx_cfg_->cfg_val_set_cfgdata_size());
          
          cfg_param_cache_map_[parameter.get_name()] = {parameter.get_parameter_value(), PARAM_VALSET};
          
        } catch (const rclcpp::ParameterTypeException &e) {
          valid = false;
          RCLCPP_DEBUG(get_logger(), "cfg_val_set_from_parameter ParamterTypeException: %s", e.what());
    ...
    try {
      for (const rclcpp::Parameter &parameter: parameters){
        ...
        if ((cache_state.status == PARAM_LOADED || cache_state.status == PARAM_VALSET) 
          ...
          RCLCPP_DEBUG(get_logger(),"on_set_parameters_callback parameter: %s same in cache. Not updating.", parameter.get_name().c_str());
        } else if (is_initialising_ && cache_state.status == PARAM_INITIAL){
          RCLCPP_DEBUG(get_logger(),"on_set_parameters_callback parameter: %s initial.", parameter.get_name().c_str());
        } else {
          if (cache_state.status == PARAM_USER) {
            RCLCPP_DEBUG(get_logger(),"on_set_parameters_callback parameter: %s args/yaml supplied.", parameter.get_name().c_str());
          }
          result = cfg_val_set_from_parameter(parameter);
        }

        if (!result.successful) {
          RCLCPP_WARN(get_logger(), "parameter %s not set - %s", parameter.get_name().c_str(), result.reason.c_str());
          break;
        } else {
          RCLCPP_INFO(get_logger(), "parameter set %s: %s", parameter.get_name().c_str(), parameter.value_to_string().c_str());
        }
      }

      if (!is_initialising_ && result.successful && ubx_cfg_->cfg_val_set_cfgdata_size() > 0) {
        RCLCPP_DEBUG(get_logger(), "on_set_parameters_callback sending asynchronous cfg_val_set_poll for parameter changes... ");
      ...
      RCLCPP_ERROR(get_logger(), "Exception: %s", 
      e.what());
      result.successful = false;
      result.reason = e.what();
    }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void hot_start_callback (const std::shared_ptr<ublox_ubx_interfaces::srv::HotStart::Request> request,
                             std::shared_ptr<ublox_ubx_interfaces::srv::HotStart::Response> response) {
    RCLCPP_WARN(get_logger(), "hot_start service called reset_type: %s", ubx::to_hex(request->reset_type).c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void warm_start_callback (const std::shared_ptr<ublox_ubx_interfaces::srv::WarmStart::Request> request,
                             std::shared_ptr<ublox_ubx_interfaces::srv::WarmStart::Response> response) {
    RCLCPP_WARN(get_logger(), "warm_start service called reset_type: %s", ubx::to_hex(request->reset_type).c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void cold_start_callback (const std::shared_ptr<ublox_ubx_interfaces::srv::ColdStart::Request> request,
                             std::shared_ptr<ublox_ubx_interfaces::srv::ColdStart::Response> response) {
    RCLCPP_WARN(get_logger(), "cold_start service called reset_type: %s" , ubx::to_hex(request->reset_type).c_str());
  ...
  void ublox_in_callback(libusb_transfer *transfer_in) {
    ...
    if (num_transfer_in_queued > 1) {
      RCLCPP_WARN(get_logger(), "too many transfer in transfers are queued (%lu)", num_transfer_in_queued);
    }
  }
  ...
  void ublox_out_callback(libusb_transfer *transfer_out) {
    ...
    RCLCPP_DEBUG(this->get_logger(), "out - status: %d length: %d buf: %s", transfer_out->status, transfer_out->actual_length, os.str().c_str());
  }
...
private:
      ...
      try {
        ...
        switch (f.frame_type) {
          ...
          case FrameType::frame_out:
            ...
            break;
          default:
            RCLCPP_ERROR(get_logger(),"Unknown ubx_queue frame_type: %d - doing nothing", f.frame_type);
        }
      } catch (const ubx::UbxValueException& e) {
  ...
  void ubx_queue_frame_in(ubx_queue_frame_t *f) {
    RCLCPP_DEBUG(get_logger(), "ubx class: 0x%02x id: 0x%02x in payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, f->ubx_frame->payload_to_hex().c_str());

    if (!ubx_frame_checksum_check(f->ubx_frame)) {
      RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x in checksum failed! Frame ignored ...", f->ubx_frame->msg_class, f->ubx_frame->msg_id);
      return;
    }
    ...
    switch (f->ubx_frame->msg_class) {
      ...
      case ubx::UBX_NAV:
        ...
        break;
      default:
        RCLCPP_WARN(get_logger(),"ubx class: 0x%02x unknown ... doing nothing", f->ubx_frame->msg_class);
    }
  }

  UBLOX_DGNSS_NODE_LOCAL
  void ubx_queue_frame_out(ubx_queue_frame_t *f) {
    RCLCPP_DEBUG(get_logger(), "ubx class: 0x%02x id: 0x%02x out payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, f->ubx_frame->payload_to_hex().c_str());

    if (!ubx_frame_checksum_check(f->ubx_frame)) {
      RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x checksum failed! Frame ignored ...", f->ubx_frame->msg_class, f->ubx_frame->msg_id);
      return;
    }
    ...
    switch (f->ubx_frame->msg_class) {
      ...
      case ubx::UBX_NAV:
        ...
        break;
      default:
        RCLCPP_WARN(get_logger(),"unkown class: 0x%02x unknown ... doing nothing", f->ubx_frame->msg_class);
    }
  }
  ...
  void ubx_cfg_out_frame(ubx_queue_frame_t *f) {
    switch (f->ubx_frame->msg_id) {
      case ubx::UBX_CFG_VALGET:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x cfg val get poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_CFG_VALSET:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x cfg val set sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_CFG_RST:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x cfg rst sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      default:
        RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x unknown ... doing nothing", f->ubx_frame->msg_class, f->ubx_frame->msg_id);
    }
  }
  ...
  void ubx_mon_out_frame(ubx_queue_frame_t *f) {
    switch (f->ubx_frame->msg_id) {
      case ubx::UBX_MON_VER:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x mon ver poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      default:
        RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x unknown ... doing nothing", f->ubx_frame->msg_class, f->ubx_frame->msg_id);
    }
  ...
  void ubx_nav_out_frame(ubx_queue_frame_t *f) {
    switch (f->ubx_frame->msg_id) {
      ...
      case ubx::UBX_NAV_POSECEF:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav pos ecef poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_NAV_HPPOSECEF:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav hp pos ecef poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_NAV_POSLLH:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav pos llh poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_NAV_HPPOSLLH:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav hp pos llh poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      ...
      case ubx::UBX_NAV_STATUS:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav status poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_NAV_RELPOSNED:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav relposned poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_NAV_TIMEUTC:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav timeutc poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_NAV_VELECEF:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav velecef poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      case ubx::UBX_NAV_VELNED:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav velned poll sent to usb device", f->ubx_frame->msg_class, f->ubx_frame->msg_id );
        break;
      default:
        RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x unknown ... doing nothing", f->ubx_frame->msg_class, f->ubx_frame->msg_id);
    }
  ...
  void ubx_ack_frame(ubx_queue_frame_t *f) {
    ...
    switch (f->ubx_frame->msg_id) {
      case ubx::UBX_ACK_ACK:
        payload_ack_ack = std::make_shared<ubx::ack::AckAckPayload>(f->ubx_frame->payload, f->ubx_frame->length);
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x ack ack payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload_ack_ack->to_string().c_str());
        break;
      case ubx::UBX_ACK_NAK:
        payload_ack_nak = std::make_shared<ubx::ack::AckNakPayload>(f->ubx_frame->payload, f->ubx_frame->length);
        RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x ack nak payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload_ack_nak->to_string().c_str());
        // TODO investigate how to get a message about why it returned a nak 
  ...
  void ubx_cfg_in_frame(ubx_queue_frame_t *f) {
    switch (f->ubx_frame->msg_id) {
      case ubx::UBX_CFG_VALGET:
        ...
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x cfg polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, ubx_cfg_->cfg_val_get_payload()->to_string().c_str());
  ...
  void ubx_mon_in_frame(ubx_queue_frame_t *f) {
    switch (f->ubx_frame->msg_id) {
      case ubx::UBX_MON_VER:
        ...
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x mon ver polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, ubx_mon_->ver()->payload()->to_string().c_str());
        break;
      default:
        RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x unknown ... doing nothing", f->ubx_frame->msg_class, f->ubx_frame->msg_id);
    }
  ...
  void ubx_inf_in_frame(ubx_queue_frame_t *f) {
    ...
    switch (f->ubx_frame->msg_id) {
      case ubx::UBX_INF_DEBUG:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x inf debug payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, ubx_inf_->debug()->payload()->to_string().c_str());
        break;
      case ubx::UBX_INF_ERROR:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x inf error payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, ubx_inf_->error()->payload()->to_string().c_str());
        break;
      case ubx::UBX_INF_NOTICE:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x inf notice payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, ubx_inf_->notice()->payload()->to_string().c_str());
        break;
      case ubx::UBX_INF_TEST:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x inf test payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, ubx_inf_->test()->payload()->to_string().c_str());
        break;
      case ubx::UBX_INF_WARNING:
        RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x inf warning payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, ubx_inf_->warning()->payload()->to_string().c_str());
        break;
      default:
        RCLCPP_WARN(get_logger(), "ubx class: 0x%02x id: 0x%02x unknown ... doing nothing", f->ubx_frame->msg_class, f->ubx_frame->msg_id);
    }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_vel_ecef_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::velecef::NavVelECEFPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav velecef polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_vel_ned_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::velned::NavVelNEDPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav velned polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_time_utc_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::timeutc::NavTimeUTCPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav timeutc polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_rel_pos_ned_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::relposned::NavRelPosNedPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav relposned polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  void ubx_nav_pvt_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::pvt::NavPvtPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav pvt polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_pos_llh_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::posllh::NavPosLLHPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav pos llh polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_pos_ecef_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::posecef::NavPosECEFPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav pos ecef polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_odo_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::odo::NavOdoPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav odo polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_hp_pos_llh_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::hpposllh::NavHPPosLLHPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav hp pos llh polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_hp_pos_ecef_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::hpposecef::NavHPPosECEFPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav hp pos ecef polled payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_status_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::status::NavStatusPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav status payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_eoe_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::eoe::NavEOEPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav eoe payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_dop_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::dop::NavDOPPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav dop payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void ubx_nav_clock_pub(ubx_queue_frame_t *f, std::shared_ptr<ubx::nav::clock::NavClockPayload> payload){
    RCLCPP_INFO(get_logger(), "ubx class: 0x%02x id: 0x%02x nav clock payload - %s", f->ubx_frame->msg_class, f->ubx_frame->msg_id, payload->to_string().c_str());
  ...
  // UBLOX_DGNSS_NODE_LOCAL
  // void ublox_gnss_init() {

  //   ubx_cfg_->cfg_val_get_keys_clear();
  //   ubx_cfg_->cfg_set_val_get_layer_ram();
  //   ubx_cfg_->cfg_val_get_key_append(ubx::cfg::CFG_UART1OUTPROT_UBX);

  //   std::shared_ptr<ubx::cfg::valget_payload_t> valget_payload_polled;
  //   try {
  //     valget_payload_polled = ubx_cfg_->ValGetPollRequest();

  //     RCLCPP_INFO(this->get_logger(), "poll_frame - %s", ubx_cfg_->cfg_val_get_frame_poll()->to_hex().c_str());
  //     RCLCPP_INFO(this->get_logger(), "polled_frame - %s", ubx_cfg_->cfg_val_get_frame()->to_hex().c_str());
  //     RCLCPP_INFO(this->get_logger(), "valget_payload_polled - %s", valget_payload_polled->to_string().c_str());
  
  //   } catch (const ubx::UbxAckNackException& e) {
  //     std::string poll_s, valget_payload_s, polled_s;
  //     if (ubx_cfg_->cfg_val_get_frame_poll() != nullptr) {
  //       poll_s = ubx_cfg_->cfg_val_get_frame_poll()->to_hex();
  //       valget_payload_s = ubx_cfg_->cfg_val_get_payload()->to_hex();
  //     } else {
  //       poll_s = "*no poll frame*";
  //     }

  //     if (ubx_cfg_->cfg_val_get_frame() != nullptr) {
  //       polled_s = ubx_cfg_->cfg_val_get_frame()->to_hex();
  //     } else {
  //       polled_s = "*no polled frame*";
  //     }
  //     RCLCPP_ERROR(this->get_logger(), "UbxAckNackException: %s poll frame: %s payload: %s polled frame: %s", e.what(),poll_s.c_str(), valget_payload_s.c_str(), polled_s.c_str());
  //     // rclcpp::shutdown();
  //   } catch (const usb::UsbException& e) {
  //     RCLCPP_WARN(this->get_logger(), "usb write_buf exception: %s", e.what());
  //   } catch (const std::runtime_error& e){
  //     RCLCPP_ERROR(this->get_logger(), "runtime_error: %s", e.what());
  //   } catch (const std::exception& e){
  ...
  void ublox_val_set_all_cfg_items_async() {
    ...
    if (trans_start) {
      RCLCPP_INFO(get_logger(), "cfg_val_set_poll_async ... end transaction ... %s", item_list.c_str());
